...
void sortDigitsAscending() {        //sort the digits in an ascending order for future operations
    if (n1 > n2) {
        swap(&n1, &n2);
        if (n1 > n3) {
            swap(&n1, &n3);
...
void sortDigitsDescending() {    //sort the digits in a descending order in a hardcoded way
    swap(&n1, &n4);
    swap(&n3, &n2);
}

int
returnNumber(int n1, int n2, int n3, int n4) { //composes a number out of four digits
    int output = 0;
    output = output * 10 + n1;
    output = output * 10 + n2;
    output = output * 10 + n3;
