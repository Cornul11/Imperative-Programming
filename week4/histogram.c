...
void computeFrequencies(int grades[], int freq[10], int length) {    //computes how often each grade is introduced
    for (int i = 0; i < length; i++) {
        freq[grades[i] - 1]++;
    }
}

int arrayMax(int length, int arr[]) {    //return the maximum of an array
    int i, max = arr[0];
    for (i = 1; i < length; i++) {
        if (arr[i] > max) {
            max = arr[i];
...
void printHistogram(int freq[10]) {
    for (int i = arrayMax(10, freq) - 1; i >= 0; i--) {    //number of rows = maximal frequency
