...
int modExp(int val, int pow, int mod) {    //computes the modular exponentiation without overflow using recursion
    if (pow == 0) {
        return (1);
    } else if (pow % 2 == 0) {
...
int isPrime(int num) {    //traditional method of checking if a number is prime or not
    if (num <= 1) {
        return (0);
    }
